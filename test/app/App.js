...
describe('containers', () => { // eslint-disable-line
  describe('App', () => { // eslint-disable-line
    it('should display initial heart', () => { // eslint-disable-line
      const { heartNum } = setup();
      expect(heartNum[0].textContent).toMatch(/^0/);
    });

    it('should display updated pinglun1 after pinglun1 button click', () => { // eslint-disable-line
      const { buttons, p, app } = setup(); // eslint-disable-line
    ...
    it('should display updated hf2 after hf2 button click', ()  => { // eslint-disable-line
      const { app } = setup();
      const realDom = findDOMNode(app);
      const plbtn = realDom.querySelectorAll('.plbtn');
